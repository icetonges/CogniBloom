...
export function NoteEditor({ note, onClose, onSave }: NoteEditorProps) {
  ...
  const doSave = async (): Promise<Note> => {
    ...
    if (note) {
      const data: UpdateNoteInput = { title, content, subject: subject || undefined, tags: tagArray }
      return notesHook.updateNote(note.id, data)
    }
    const data: CreateNoteInput = { title, content, subject: subject || undefined, tags: tagArray }
    return notesHook.createNote(data)
  }
  ...
  const handleSaveAndReview = async () => {
    ...
    try {
      const saved = await doSave()
      targetId = saved.id; setSavedNoteId(targetId)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to save note')
  ...
  const masteryColor = masteryPct >= 75 ? 'text-green-500' : masteryPct >= 50 ? 'text-amber-500' : 'text-red-500'
      ...
      <Card className="p-6 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Math, Science…" disabled={isSaving} />
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mb-2">
            Markdown · LaTeX ($...$) · Code blocks (```lang...```)
          </p>
          ...
          {showPreview ? (
            <div className="grid grid-cols-2 gap-3">
              <textarea
                ...
                className="h-72 p-3 border border-input rounded-md font-mono text-sm bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring resize-none"
                disabled={isSaving}
              />
              <div className="h-72 p-3 border border-border rounded-md bg-muted/30 overflow-auto">
                {content.trim() ? (
      ...
      {review && (
        <Card className="p-5 space-y-4 border-primary/30 bg-primary/5">
          <div className="flex items-center gap-2">
            ...
            <span className={cn('ml-auto text-sm font-bold', masteryColor)}>{masteryPct}% mastery</span>
          </div>
            ...
            <div>
              ...
              <ul className="space-y-1">
                {review.keyPoints.map((p, i) => (
                  <li key={i} className="text-sm flex items-start gap-1.5"><span className="text-primary mt-0.5">•</span> {p}</li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-1">
                {review.recommendations.map((r, i) => (
                  <li key={i} className="text-sm flex items-start gap-1.5"><span className="text-amber-500 mt-0.5">→</span> {r}</li>
                ))}
              </ul>
            </div>
          </div>
          {review.weakAreas.length > 0 && (
            <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-3">
              ...
              {review.weakAreas.map((w, i) => <p key={i} className="text-sm text-muted-foreground">⚠ {w}</p>)}
            </div>
          )}
          <Button size="sm" variant="outline" onClick={onClose} className="w-full">Done — close editor</Button>
        </Card>
      )}
    </div>
  )
